...
class UtilisateurManager extends MainManager{
	...
	public function getINE($ine){
        $req = "SELECT * FROM utilisateur WHERE ine = :ine";
        $stmt = $this->getBdd()->prepare($req);
    ...
    public function bdCreerCompte($login, $ine, $nom, $prenom, $adresse, $ville, $passwordCrypte,$mail,$clef,$image,$role){
        $req = "INSERT INTO utilisateur (login, ine, nom, prenom, adresse, ville, password, mail, est_valide, role, clef, image)
        VALUES (:login,:ine, :nom, :prenom, :adresse, :ville, :password, :mail, 0, :role, :clef, :image)";
        $stmt = $this->getBdd()->prepare($req);
        $stmt->bindValue(":login",$login,PDO::PARAM_STR);
        $stmt->bindValue(":ine",$ine,PDO::PARAM_STR);
	...
	public function verifINEDisponible($ine){
        $utilisateur = $this->getINE($ine);
        return empty($utilisateur);
    }
