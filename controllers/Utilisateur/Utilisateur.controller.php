...
class UtilisateurController extends MainController{
    ...
    public function validation_creerCompte($login,$ine, $nom, $prenom, $adresse, $ville, $password,$mail){
        if($this->utilisateurManager->verifLoginDisponible($login)){
			if($this->utilisateurManager->verifINEDisponible($ine)){
				$passwordCrypte = password_hash($password,PASSWORD_DEFAULT);
				$clef = rand(0,9999);
				if($this->utilisateurManager->bdCreerCompte($login,$ine, $nom, $prenom, $adresse, $ville,$passwordCrypte,$mail,$clef,"profils/profil.png","utilisateur")){
				...
				Toolbox::ajouterMessageAlerte("Le numéro INE est déjà utilisé !", Toolbox::COULEUR_ROUGE);
				header("Location: ".URL."creerCompte");
			}
        } else {
